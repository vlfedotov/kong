...
local function load(path, custom_conf)
  ...
  do
    ...
    local proxy_listeners, err = parse_listeners(conf.proxy_listen, { "proxy_protocol", "mock" })
    if err then
    ...
    for _, listener in ipairs(proxy_listeners) do
      local i = listener.mock and 2 or 1

      proxy_servers[i].enabled = true

      if listener.ssl then
        proxy_servers[i].ssl_enabled = true
        conf.proxy_ssl_enabled       = true
      end

      table.insert(proxy_servers[i].listeners, listener)
    end

    for i = 2, 1, - 1 do
      if not proxy_servers[i].enabled then
        table.remove(proxy_servers, i)
      end
    end

    conf.proxy_servers = proxy_servers
