...
local function client_requests(n, host, headers)
  local oks, fails = 0, 0
  local last_status
  for _ = 1, n do
    local client = helpers.proxy_client()
    headers = utils.deep_copy(headers) or {}
    headers["Host"] = assert(host)
...
do
  ...
  patch_api = function(api_name, data)
    assert.same(200, api_send("PATCH", "/apis/" .. api_name, data))
  end
...
for mode, localhost in pairs(localhosts) do
  ...
  describe("#" .. mode, function()
    ...
    describe("#healthchecks", function()
      ...
      it("#wait perform active health checks -- up then down", function()
      ...
      it("#wait perform active health checks -- automatic recovery", function()
      ...
      it("#wait perform active health checks -- can detect before any proxy traffic", function()
